...
//³õÊ¼»¯PA6ÎªÏÂÀ­ÊäÈë		    
//¶ÁÃþ³ö¸ÐÓ¦×´Ì¬(´¥Ãþ¸ÐÓ¦Ê±Êä³ö¸ßµçÆ½ÐÅºÅ)
...
//åˆ¤æ–­ä¸­æ–­æŽ¥æ”¶çš„æ•°ç»„æœ‰æ²¡æœ‰åº”ç­”åŒ„1¤7
//waittimeä¸ºç­‰å¾…ä¸­æ–­æŽ¥æ”¶æ•°æ®çš„æ—¶é—´ï¼ˆå•ä½„1¤71msï¼„1¤7
//è¿”å›žå€¼ï¼šæ•°æ®åŒ…é¦–åœ°å€
static uint8_t *JudgeStr(uint16_t waittime)
{
	...
	{
		delay_ms(1);
		if(USART1_RX_STA&0X8000 || syspara_t.uart1_rx_data==1)//Â½Ã“ÃŠÃ•ÂµÂ½Ã’Â»Â´ÃŽÃŠÃ½Â¾Ã
		{
...
//åŠŸèƒ½:æŽ¢æµ‹æ‰‹æŒ‡ï¼ŒæŽ¢æµ‹åˆ°åŽå½•å…¥æŒ‡çº¹å›¾åƒå­˜äºŽImageBufferã€„1¤7 
//æ¨¡å—è¿”å›žç¡®è®¤å­„1¤7
...
//åŠŸèƒ½:ä»¥CharBuffer1æˆ–CharBuffer2ä¸­çš„ç‰¹å¾æ–‡ä»¶æœç´¢æ•´ä¸ªæˆ–éƒ¨åˆ†æŒ‡çº¹åº“.è‹¥æœç´¢åˆ°ï¼Œåˆ™è¿”å›žé¡µç ã€„1¤7			
...
//¶ÁÏµÍ³»ù±¾²ÎÊý PS_ReadSysPara
//¹¦ÄÜ:  ¶ÁÈ¡Ä£¿éµÄ»ù±¾²ÎÊý£¨²¨ÌØÂÊ£¬°ü´óÐ¡µÈ)
//²ÎÊý:  ÎÞ
//ËµÃ÷:  Ä£¿é·µ»ØÈ·ÈÏ×Ö + »ù±¾²ÎÊý£¨16bytes£©
...
//		  è‹¥æœç´¢åˆ°ï¼Œåˆ™è¿”å›žé¡µç ,è¯¥æŒ‡ä»¤å¯¹äºŽçš„ç¡®å­˜åœ¨äºŽæŒ‡çº¹åº“ä¸­ ï¼Œä¸”ç™»å½•æ—¶è´¨é‡„1¤7
//		  å¾ˆå¥½çš„æŒ‡çº¹ï¼Œä¼šå¾ˆå¿«ç»™å‡ºæœç´¢ç»“æžœã¢ã„1¤7
//å‚æ•°:  BufferIDï¼„1¤7 StartPage(èµ·å§‹é¡„1¤7)ï¼ŒPageNumï¼ˆé¡µæ•°ï¼‰
//è¯´æ˜Ž:  æ¨¡å—è¿”å›žç¡®è®¤å­„1¤7+é¡µç ï¼ˆç›¸é…æŒ‡çº¹æ¨¡æ¿ï¼‰
...
//¹¦ÄÜ£º¶ÁÓÐÐ§Ä£°å¸öÊý
//²ÎÊý: ÎÞ
//ËµÃ÷: Ä£¿é·µ»ØÈ·ÈÏ×Ö+ÓÐÐ§Ä£°å¸öÊýValidN
...
//	MYUSART_SendData(0X00);	
//	delay_ms(200);
//	if(USART2_RX_STA&0X8000)//½ÓÊÕµ½Êý¾Ý
//	{		
//		if(//ÅÐ¶ÏÊÇ²»ÊÇÄ£¿é·µ»ØµÄÓ¦´ð°ü				
...
void Press_ReadFingerprint_Data(void)
{
   ...
   //  if(syspara_t.ps_serch_getimage !=0)
    ...
    if(run_t.Confirm_newPassword==0 && run_t.panel_lock==0 && syspara_t.ps_readEeprom_data >0 && syspara_t.ps_thefirst_input_fp ==2){
       ...
       while(syspara_t.ps_serch_getimage){
		...
		if(syspara_t.ps_serch_getimage==0x00)//if(ensure==0x00)//èŽ·å–å›¾åƒæˆåŠŸ 
		{	
			...
			syspara_t.ps_serch_genchar=PS_GenChar(CharBuffer1);
	         ...
	         syspara_t.PS_read_template=2;//receive data is 16bytes by USART1 port
					...
					{				
		FP_SEARCH:		syspara_t.PS_check_fp_success =1;//OLED_ShowCH(0,2,"  æŒ‡çº¹éªŒè¯æˆåŠŸ  ");				
   ...
   /*************************************************************/
	if(syspara_t.ps_readEeprom_data==0 && run_t.Confirm_newPassword==0  && run_t.inputNewPassword_Enable==0){
    	 ...
    	 if(syspara_t.ps_serch_getimage!=0)
	           syspara_t.ps_serch_getimage=PS_GetImage();
		   if(syspara_t.ps_serch_getimage==0x00)//if(ensure==0x00)//é‘¾å³°å½‡é¥æƒ§å„šéŽ´æ„¬å§› 
		   {	
